...
from gainer.gainer_model import GainerModel
from gainer.utils.generate_mask import generate_reference_pcd, create_mask
...
class DatasetRender(BaseRender):
    ...
    """Optional path to a mask file used during rendering."""
    ...
    def main(self):
        ...
        def update_config(config: TrainerConfig) -> TrainerConfig:
            ...
            if self.mask_path is not None:
                    if hasattr(config.pipeline, "mask_path"):
                        config.pipeline.mask_path = self.mask_path
            return config
        ...
        # Editing takes place each step so knn needs to be calculated accordingly
        pipeline.model.field.mlp_base.encoder.recalc_interval = 1

        # Set occ grid enocoder to sample from the whole space for editing
        assert isinstance(pipeline.model, GainerModel), "DatasetRender only works with GainerModel"
        ...
        for split in self.split.split("+"):
            ...
            plys_path = config.get_base_dir() / "camera_path"
            num_edits = list(plys_path.glob("*.ply"))

            camera, batch = next(iter(dataloader))
            ...
            with Progress(
                ...
                for camera_idx in progress.track(range(len(num_edits))):

                    # Load edited pointcloud
                    assert plys_path, "Camera path directory does not exist"
                    pcd = o3d.io.read_point_cloud(str(plys_path / f"{camera_idx:05d}.ply"))
                    points = torch.from_numpy(np.asarray(pcd.points)).float().to(pipeline.device)
                    pipeline.model.field.mlp_base.encoder.means.data = points
                    ...
                    # Update sampling mask based on the new point cloud
                    if config.pipeline.model.background_color != "random":
                        reference_pcd = generate_reference_pcd(gt_batch["rgb"], config.pipeline.datamanager.dataparser.num_points)
                        new_sampling_mask = create_mask(gt_batch["rgb"], reference_pcd, pcd)
                        pipeline.model.sampler.update_sampling_mask(new_sampling_mask.to(pipeline.model.device).flatten())
